function model = gppwSgdLearn(model, opt)
%GPPWSGDLEARN model = gppwSgdLearn(model, opt)
%   
% Stochastic gradient descent (SGD) training for a pairwise regression model.
% The standard SGP is applied where we use the update that has the momentum term:
% u_{t+1} = u_t + delta_{t}
...
for n=1:opt.epochs
  ...
  for i=1:P
    ...
    [~, g, gmean] = gppwInf(model, userid);
    model = gppwUpdate(g, gmean, userid, model, opt);
    % show sum of log likelihood to check for convergence
    if ~isempty(opt.monitorConvergence) && mod(iter, opt.monitorConvergence) == 0 
      model.objectives = [model.objectives; gppwObjective(model)];
      if opt.displayObjectives
        disp([num2str(iter) ' ' num2str(model.objectives(end))])
      end  
    end
    iter = iter + 1;
    %opt.learnRate = opt.initialLearnRate/(1+opt.initialLearnRate*0.1*iter);
  end
end
end
